...
class RoadNetwork(object):
    ...
    def add_lane(self, _from: str, _to: str, lane: AbstractLane) -> None:
        ...
        # Add two-sided connection reference
        if _from not in self.connections:
            self.connections[_from] = []
        if _to not in self.connections[_from]:
            self.connections[_from].append(_to)

        if _to not in self.connections:
            self.connections[_to] = []
        if _from not in self.connections[_to]:
            self.connections[_to].append(_from)
    ...
    def get_upcoming_lanes(self, index: LaneIndex) -> List[LaneIndex]:
        _from, _to, _id = index
        upcoming_lanes = []

        next_destination_nodes = self.connections[_to]
        # print(next_destination_nodes)
        for ndn in next_destination_nodes:
            if ndn == _from:
                continue
            if ndn not in self.graph[_to]:
                continue
            for lane_number, lane_object in enumerate(self.graph[_to][ndn]):
                upcoming_lanes.append((_to, ndn, lane_number))
        return upcoming_lanes

    # Retrieves all LaneIndices of the corresponding road including the given LaneIndex.
    def get_road_lanes(self, index: LaneIndex) -> List[LaneIndex]:
        parallel_lanes = []
        _from, _to, _id = index
        for lane_number in range(len(self.graph[_from][_to])):
            parallel_lanes.append((_from, _to, lane_number))
        return parallel_lanes

    def get_nearest_lane_index(self, index: LaneIndex, position: np.ndarray) -> LaneIndex:
        previous_lane = self.get_lane(index)
        previous_heading = previous_lane.lane_heading(position)
        indexes, distances = [], []
        for _from, to_dict in self.graph.items():
            for _to, lanes in to_dict.items():
                for _id, l in enumerate(lanes):
    ...
    # Retrieves the closest lane for a given world position based on the current LaneIndex
    def get_nearest_lane(self, index: LaneIndex, position: np.ndarray) -> LaneIndex:

        # there are upcoming roads and there are parallel lanes

        upcoming = self.get_upcoming_lanes(index)
        parallel = self.get_road_lanes(index)
        candidates = upcoming + parallel

        prime_candidate = None
        best_distance = np.inf
        for candidate in candidates:
            # print(candidate)
            distance = self.get_lane(candidate).distance(position)
            if distance < best_distance:
                prime_candidate = candidate
                best_distance = distance
        return prime_candidate

    def get_closest_lane(self, position: np.ndarray, heading: Optional[float] = None) -> AbstractLane:
        """
        ...
        :return: the index of the closest lane.
        """
        return self.get_lane(self.get_closest_lane_index(position, heading))
    ...
    def random_lane_index(self, np_random: np.random.RandomState) -> LaneIndex:
        ...
        _id = np.random.randint(len(self.graph[_from][_to]))
